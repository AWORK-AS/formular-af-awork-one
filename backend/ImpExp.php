...
class ImpExp extends Base {
	...
	public function settings_import() {
		...
		if ( 'json' !== $extension ) {
			\wp_die( \esc_html__( 'Please upload a valid .json file', CFA_TEXTDOMAIN ) );
		}

		$import_file = $_FILES[ 'cfa_import_file' ][ 'tmp_name' ]; //phpcs:ignore WordPress.Security.ValidatedSanitizedInput

		if ( empty( $import_file ) ) {
			\wp_die( \esc_html__( 'Please upload a file to import', CFA_TEXTDOMAIN ) );
		}
		...
		new \WP_Error(
				...
				\__( 'Failed to import the settings.', CFA_TEXTDOMAIN )
			);
	}
