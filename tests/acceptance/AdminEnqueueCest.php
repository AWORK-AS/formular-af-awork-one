...
class AdminEnqueueCest {
	...
	function enqueue_admin_scripts(AcceptanceTester $I) {
		...
		$I->seeInPageSource('contact-form-app-settings-script');
		$I->seeInPageSource('contact-form-app-admin-script');
	}

	function enqueue_admin_styles(AcceptanceTester $I) {
		$I->wantTo('access to the plugin settings page and check the style enqueue');
		$I->amOnPage('/wp-admin/admin.php?page=contact-form-app');
		$I->seeInPageSource('contact-form-app-settings-styles-css');
		$I->seeInPageSource('contact-form-app-admin-styles-css');
	}
